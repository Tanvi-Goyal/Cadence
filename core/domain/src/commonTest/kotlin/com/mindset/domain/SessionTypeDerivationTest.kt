...
private fun entryDetail(metric: MetricType, segmentKey: String? = null): ExerciseEntryDetail = ExerciseEntryDetail(
    entry =
    ExerciseEntry(
        id = "e-${metric.name}",
        blockId = "b",
        exerciseId = "x-${metric.name}",
        orderIndex = 0,
        targetSets = null,
        restMs = null,
...
class SessionTypeDerivationTest {
    ...
    fun all_strength_exercises_derive_strength() {
        assertEquals(
            ...
            deriveSessionType(
                detail(entryDetail(MetricType.WEIGHT_REPS), entryDetail(MetricType.REPS_ONLY)),
            ),
        )
    }
    ...
    fun all_conditioning_exercises_derive_conditioning() {
        assertEquals(
            ...
            deriveSessionType(
                detail(entryDetail(MetricType.DISTANCE_TIME), entryDetail(MetricType.DURATION)),
            ),
        )
    }
    ...
    fun strength_plus_conditioning_derives_mixed() {
        assertEquals(
            SessionType.MIXED,
            deriveSessionType(
                detail(entryDetail(MetricType.WEIGHT_REPS), entryDetail(MetricType.DISTANCE_TIME)),
            ),
        )
    }
