...
private fun exercise(metric: MetricType) = Exercise(
    id = "x", name = "X", modality = Modality.STRENGTH, defaultMetric = metric, hyroxStation = null,
...
private fun set(
    reps: Int? = null, loadKg: Double? = null, timeSec: Int? = null,
    distanceM: Int? = null, calories: Int? = null,
) = SetEntry(
    id = "s", exerciseEntryId = "e", setNumber = 1,
    reps = reps, loadKg = loadKg, timeSec = timeSec, distanceM = distanceM, calories = calories, rpe = null,
...
private fun pr(kind: PrKind, value: Double, bucket: Int? = null) = PersonalRecord(
    id = "pr-${kind.name}-$bucket", exerciseId = "x", kind = kind, value = value, distanceBucketM = bucket,
    achievedAt = EPOCH, sourceSetId = "s0", createdAt = EPOCH, updatedAt = EPOCH, deletedAt = null,
)
...
class PbDetectionTest {
    ...
    fun first_weight_reps_set_records_both_e1rm_and_max_weight() {
        val result = detectPrs(exercise(MetricType.WEIGHT_REPS), set(reps = 5, loadKg = 100.0), emptyList())
    ...
    fun equal_e1rm_and_weight_are_not_new_prs() {
        val current = listOf(pr(PrKind.EST_1RM, epleyOneRepMax(100.0, 5)), pr(PrKind.MAX_WEIGHT, 100.0))
    ...
    fun heavier_or_stronger_beats_the_standing_record() {
        val current = listOf(pr(PrKind.EST_1RM, epleyOneRepMax(100.0, 5)), pr(PrKind.MAX_WEIGHT, 100.0))
        val result = detectPrs(exercise(MetricType.WEIGHT_REPS), set(reps = 3, loadKg = 105.0), current)
    ...
    fun reps_only_tracks_max_reps_strictly() {
        ...
        assertEquals(listOf(PrKind.MAX_REPS), detectPrs(ex, set(reps = 20), emptyList()).map { it.kind })
        assertTrue(detectPrs(ex, set(reps = 15), listOf(pr(PrKind.MAX_REPS, 15.0))).isEmpty(), "tie is not a PR")
        assertEquals(21.0, detectPrs(ex, set(reps = 21), listOf(pr(PrKind.MAX_REPS, 20.0))).first().value)
    }
    ...
    fun distance_time_records_best_time_per_bucket_lower_wins() {
        ...
        assertTrue(detectPrs(ex, set(distanceM = 1000, timeSec = 240), current).isEmpty(), "equal time not a PR")
        assertEquals(230.0, detectPrs(ex, set(distanceM = 1000, timeSec = 230), current).first().value, )
        // A different distance is a different bucket → still a PR against a 1000 m record.
        assertEquals(2000, detectPrs(ex, set(distanceM = 2000, timeSec = 500), current).first().distanceBucketM)
    }
    ...
    fun calories_tracks_max_calories() {
        ...
        assertTrue(detectPrs(ex, set(calories = 70), listOf(pr(PrKind.MAX_CALORIES, 70.0))).isEmpty())
    }

    @Test
    fun duration_has_no_default_record() {
        assertTrue(detectPrs(exercise(MetricType.DURATION), set(timeSec = 120), emptyList()).isEmpty())
    }

    @Test
    fun incomplete_actuals_yield_no_record() {
        // WEIGHT_REPS with a missing load can't produce a lift PR.
        assertTrue(detectPrs(exercise(MetricType.WEIGHT_REPS), set(reps = 5), emptyList()).isEmpty())
    }
}
